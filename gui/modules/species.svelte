<script>
  ...
  const entity = 'smartyellow/species';
  const moduleName = 'smartyellow/species';
  const pluginName = 'smartyellow/species';
  const readonly = user.cannot(pluginName + '/editSpecies');
  ...
  onMount(async function () {
    try {
      if (id) {
        ...
        settings = await api.get('/species/settings');
        // get item, form and log for specified id
        ({ item, form, log } = isNew ? await api.post('/species', {}, { init: true }) : await api.get('/species/' + id));
        ...
        filters = await api.get('/species/filters');
        gridOptions.columns = await api.get('/species/formats');
      ...
      console.log('Failed to fetch species', e);
    }
  });
  ...
  async function saveChanges() {
    if (focused) {
      // save changes for species in form
      savebar.start();
      const result = isNew ?
        await api.post('/species', item) :
        await api.put('/species/' + item.id, item);
      savebar.stop(result);
      if (!result.errors) {
        log = result.log;
        // Once saved, we need to use PUT request to update newly created species
        isNew = false;
        // Update tab title
        dispatch('tabchanged', { title: item.name });
      }
  ...
  async function submitFilters({ detail }) {
    ...
    items = await api.post('/species/search', detail);
  }
  ...
  async function deleteItem() {
    if (item && item.id) {
      try {
        ...
        dispatch('close');
        await api.delete('/species/' + item.id);
      }
      catch (e) {
        console.log(e);
      }
...
{#if mounted}
  ...
  {#if item}
    ...
    <Toolbar large="768" fill="true">
      {#if user.can(pluginName + '/editSpecies')}
      ...
      {/if}
      {#if log && log.created && user.can(pluginName + '/deleteSpecies')}
  ...
  {:else}
    ...
    <Toolbar large="768">
      ...
      />
      {#if user.can(pluginName + '/createSpecies')}
        <ToolbarButton {icon} on:click={e => openItem(e, true)} title={translate('new', language)} hint={translate('Create new species', language)} />
      {/if}
    </Toolbar>
